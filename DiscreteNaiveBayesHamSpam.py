...
#creating Bernoulli model for hams
...
#pretty inefficient for runtime, but efficient in terms of coding to make Bag of Words then adjust to Bernoulli
hamsdm[hamsdm>0] = 1

#creating Bernoulli model for spams
...
#making discrete Naive Bayes model
...
#calculate probabilities, alpha is 1
for word, count in wordHamCounts.items():
    features['ham'][word] = math.log((count + 1)/(hamCount + 2))

for word, count in wordSpamCounts.items():
    features['spam'][word] = math.log((count + 1)/(spamCount+2))
...
for file in os.listdir(ham1Test):
    ...
    txt = [*set(txt)] #remove duplicate words so that each word is counted once (present or not present)
